...
export default function createTransformalizer(baseOptions = {}) {
   ...
   */
  function transform({ name, source, options: opts, local }) {
    if (!isString(name)) {
    ...
    const data = transformSource({ docSchema, source, options, include, local })
  ...
  function transformSource(args) {
    const { docSchema, source, options: opts, include, local } = args
    if (Array.isArray(source)) {
      return source.map(data => transformData({ docSchema, source, options: opts, data, include, local }))
    }
  ...
  function transformData(args) {
    const { docSchema, source, options, data, include, local, _type, _id } = args
    ...
    if (isFunction(docSchema.schema.data.dataSchema)) {
      const name = docSchema.schema.data.dataSchema({ source, data, options, local })
    ...
    const state = {}
    const params = { dataSchema, source, options, data, state, local }
  ...
  function transformRelationshipData(args) {
    ...
    const { local, name, data, included, meta } = item
